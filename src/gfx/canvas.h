...
public:
    ...
    // Dashed strokes, for the ONE thing in this repository that needs them: jack-graph draws the
    // connection you are dragging as a dashed bezier and the connections that exist as solid
    // ones, and that difference is the whole signal that the cable in your hand is not connected
    // yet (GraphCanvas.cpp:441 in the GTK build). No sibling project has a dash API because none
    // of them has a provisional anything.
    //
    // Lengths are in LOGICAL units, like every other length here, so a dash does not become four
    // times longer at scale 2. Cairo's dash state is part of the context and outlives the stroke
    // that set it, so clearDash() is not optional and every setDash() has one -- a dash left on
    // by a preview that returned early would next show up on a solid cable.
    void setDash(const float *pattern, int n);
    void clearDash();
    ...
    // Fill a closed polygon from n (x, y) pairs. THE ONE ADDITION to this file across the three
    // projects that share it, and it is here because there is no way to fake it: the speaker
    // icon's cone is a triangle, and approximating one with stacked fillRects produces visible
    // stair-steps at every scale. Uses the current colour, like every other fill here.
    void fillPolygon(const float *xy, int n);
