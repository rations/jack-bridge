...
void Canvas::setDash(const float *pattern, int n)
{
    if (!pattern || n <= 0) {
        clearDash();
        return;
    }
    // cairo_set_dash takes doubles and an on/off alternation; the float pattern is the caller's
    // because every other length in this API is a float.
    double dashes[8];
    const int count = n > 8 ? 8 : n;
    for (int i = 0; i < count; ++i)
        dashes[i] = static_cast<double>(pattern[i]);
    cairo_set_dash(mCr, dashes, count, 0.0);
}

void Canvas::clearDash()
{
    cairo_set_dash(mCr, nullptr, 0, 0.0);
}
...
void Canvas::fillPolygon(const float *xy, int n)
{
    if (!xy || n < 3)
        return;
    cairo_move_to(mCr, xy[0], xy[1]);
    for (int i = 1; i < n; ++i)
        cairo_line_to(mCr, xy[2 * i], xy[2 * i + 1]);
