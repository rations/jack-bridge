...
cairo_font_face_t *FontStack::loadFace(const std::string &path, bool bold, void **outFt)
{
    *outFt = nullptr;
    ...
    fprintf(stderr, "jack-bridge: could not load font %s (using a system fallback)\n", path.c_str());
...
bool FontStack::load(const std::string &resourceDir)
{
    ...
    if (FT_Init_FreeType(&lib) == 0)
        ...
        fprintf(stderr, "jack-bridge: FreeType failed to initialise (using system fallbacks)\n");
