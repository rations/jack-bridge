...
### Know issues. If using an external usb audio interface or any usb midi hardware (drums, keyboards etc) they can override jack-bridge if plugged in on boot. If this happens plug them in after booting. Or select USB in the devices section of the GUI. Also pressing setup in the qjackctl tends to crash unless you have opened graph or messages first.
