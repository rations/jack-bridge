...
### UNTESTED VERSIONS in branches for OpenRC (Debian / Arch) and Void runit if anyone feels adventureous
 
### Known issues. Also pressing setup in the qjackctl GUI tends to crash unless you have opened graph or messages first.
...
Download `jack-bridge-20260123.tar.gz` from releases on GitHub. Then:

```bash
tar -xf jack-bridge-20260123.tar.gz
```
```bash
cd jack-bridge-20260123
```
```bash
